...
export default async function MovieDetail({ params }) {
  ...
  try {
    const movieResponse = await getMovieDetail(params.id);
    console.log("Movie Detail API Response:", movieResponse);

    if (movieResponse && movieResponse.code === 200 && movieResponse.data) {
      movie = movieResponse.data;
    } else {
      movieError = movieResponse ? movieResponse.message : '获取电影详情失败';
      console.error("Failed to fetch movie detail:", movieResponse);
    }

  } catch (err) {
    console.error("Error fetching movie detail:", err);
    movieError = '加载电影详情时发生错误';
  }
  ...
  try {
    ...
    if (commentsResponse && commentsResponse.code === 200 && Array.isArray(commentsResponse.data)) {
      comments = commentsResponse.data;
    } else {
      commentsError = commentsResponse ? commentsResponse.message : '获取评论失败';
      console.error("Failed to fetch comments:", commentsResponse);
    }
  } catch (err) {
    console.error("Error fetching comments:", err);
    commentsError = '加载评论时发生错误';
  }
  ...
  if (movieError) {
    return (
      <div className="max-w-4xl mx-auto p-6 bg-white/80 dark:bg-gray-900/80 rounded-2xl shadow-xl mt-8 text-center text-red-500">
        <p>{movieError}</p>
        <Link href="/" className="mt-4 inline-block text-bili-blue hover:underline">返回首页</Link>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start mb-8">
        ...
        <div className="flex-1 flex flex-col gap-4">
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-gray-100 mb-2 flex items-center gap-2">
            <span role="img" aria-label="movie">🎬</span> {movie.title || '未知标题'}
          </h1>
          <div className="flex gap-2 text-sm">
            {movie.genres && Array.isArray(movie.genres) && movie.genres.map((genre, index) => (
              <span key={index} className="px-2 py-0.5 bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-200 rounded-full">
                {genre}
              </span>
            ))}
            {movie.year && <span className="text-gray-400 dark:text-gray-500">{movie.year}</span>}
          </div>
          <p className="text-gray-700 dark:text-gray-300 leading-relaxed border-l-4 border-blue-200 dark:border-blue-800 pl-4 mt-2">{movie.description || '暂无简介'}</p>
        </div>
      </div>
